...
%     inletLocations(2,:) = [domainLength/4 + 10*domainLength/60, domainLength/4 + 10*domainLength/60 + inletSize];
...
%         heatSourceLocation(2) = flameStart_x + (domainLength - flameStart_x)*(1/(1 + exp(-currTime)));
    ...
    plotResults(u_velocity, v_velocity, pressureField, temperatureField, ...
        domainLength,domainWidth,dx,dy, numRecirculationInlets);
